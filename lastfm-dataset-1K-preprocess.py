...
df_user_profile = spark.read.csv('data/lastfm-dataset-1k/userid-profile.tsv',sep='\t', header=True)
...
# parquet
save_filepath = "data/lastfm-dataset-1k/lastfm-dataset-1k.snappy.parquet"
# df_user_track.to_parquet(save_filepath, compression='snappy', index=False)

df_user_track = spark.read.parquet(save_filepath)
df_user_track.write.option("parquet.block.size", 100 * 1024 * 1024)
df_user_track.write.parquet("data/lastfm-dataset-1k_history")
...
#     "data/lastfm_userid-timestamp-artid-artname-traid-traname.tsv", sep='\t', header=None,
#     names=[
#         'user_id', 'timestamp', 'artist_id', 'artist_name', 'track_id', 'track_name'
#     ],
#     # skiprows=[
...
# print(f'Number of Records: {len(df_user_track):,}\nUnique Users: {df_user_track.user_id.nunique()}\nUnique Artist:{df_user_track.artist_id.nunique():,}')
#

# save_filepath = "data/lastfm-dataset-1k.snappy.parquet"
# df_user_track.to_parquet(save_filepath, compression='snappy', index=False)